...
DECORATOR = "@st.composite"
FUNCTION_SIGNATURE = "def test_strategy(draw) -> ..."

expected_strategy_example_function_2 = f"""{DECORATOR}
def test_example_function_1(draw) -> Tuple[int, int, int, int, str, List[int]
    n1 = draw(st.integers(min_value=n2+1, max_value=min(99, n4-1))
    n2 = draw(st.integers())
    n3 = draw(st.integers())
    n4 = draw(st.integers())
    s = draw(st.from_regex())
    lst = draw(st.lists(elements=st.integers()))"""
...
class GenerateSymbolTableTest(unittest.TestCase):
    ...
    def test_example_function_2(self) -> None:
        strategy_factory = StrategyFactory(example_function_2)
